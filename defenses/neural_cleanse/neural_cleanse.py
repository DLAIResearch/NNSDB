...
from defenses.neural_cleanse.detecting  import *
from defenses.neural_cleanse.config import get_argument
from defenses.neural_cleanse import config
import numpy as np
import sys
import json
...
def outlier_detection(l1_norm_list, idx_mapping, opt):
    ...
    if opt.to_file:
        result_path = os.path.join(opt.result, opt.dataset)
...
def main():
    ...
    if opt.dataset == "mnist" or opt.dataset == "cifar10"or opt.dataset == "imagenet":
        opt.total_label = 10
